function [W, Wtotal, Z, loopBegins, sol] = main_template_rollback(...
    formula,A,h,W0,Obs,UB,CA_flag,CE_plan, CE_onestep)
% time = clock;
% disp([ 'Started at ', ...
...
warning on
% counter examples
fCE_plan = getCE_plan(CE_plan);
fCE_onestep = getCE_onestep(CE_onestep);

% All Constraints
%F = [fInit, fDyn, fLoop, fObs, fLTL, phi==1];
F = [fInit, fDyn, fLoop, fLTL, phi==1, fCE_onestep, fCE_plan, fUB];
